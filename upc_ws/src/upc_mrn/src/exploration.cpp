...
// Relplaing variables
int PrevFrontSize=0;
int totalFrontSize=0;
int elapsed_time=0;
ros::Time prevTime;


vector<int> bannedFrontiers;
int ind_minCostFrontier;
#define MAX_EXPECTED_TIME 30
#define MAX_DIST_FRONTIER 20
#define MAX_SIZE_FRONTIER 100
#define MAX_DIFF_MAP 30
int expectedTime=0;
...
{
  bool replan = false;
  ros::Duration d = ros::Time::now() - prevTime;
  elapsed_time = int(d.toSec());
  
	// Count the total number of cells occupied (only big frontiers)
	totalFrontSize=0;
		for (int i=0; i<frontiers_.size(); i++)
		{
			if(frontiers_[i].size>min_frontier_size)
			{
			totalFrontSize += frontiers_[i].size;
			}
		}
		

  if(robot_status_==1) // Goal reached
  {
  	ROS_WARN("REPLAN: Goal Reached");
  	bannedFrontiers.clear();
	 	ROS_WARN("Clearing Banning");
  	prevTime=ros::Time::now() ;
    replan = true;
  } 
  if(fabs(totalFrontSize-PrevFrontSize)>MAX_DIFF_MAP) // Substantial difference in map
  {
 	  ROS_WARN("REPLAN: Difference in the map (%d/%d)",totalFrontSize,PrevFrontSize);
   	bannedFrontiers.clear();
	 	ROS_WARN("Clearing Banning");
 	  prevTime=ros::Time::now();
  	replan = true;
  ...
  ROS_WARN("Time elapsed:%d",elapsed_time); // Too much time
  if(elapsed_time>expectedTime)
  {
	  ROS_WARN("REPLAN: Taking too much time");
  	bannedFrontiers.push_back(ind_minCostFrontier);
  	ROS_WARN("Banning %d",ind_minCostFrontier);
  	prevTime=ros::Time::now() ;
  	replan = true;
  } 
  
 	PrevFrontSize=totalFrontSize; // Update the count of maps cells occupied
...
geometry_msgs::Pose decideGoal()
{
	...
	int i = 0;
	float distMax=0,dif_x,dif_y,dist;
	float minCost=10000,lossFunction;
	
	float maxDistFrontier=0,maxSizeFrontier=0; // To normalize terms in cost function

	// Number of big enough frontiers
	int numSizeFrontiers = 0;
	for (int i=0; i<frontiers_.size(); i++)
	{
		if(frontiers_[i].size>min_frontier_size)
		{
			...
			dif_x=frontiers_[i].free_center_point.x-robot_pose_.position.x;
			dif_y=frontiers_[i].free_center_point.y-robot_pose_.position.y;
			dist=pow((pow(dif_x,2)+pow(dif_y,2)),0.5);
		
		// Max distance to frontier
			if (maxDistFrontier<dist)
					maxDistFrontier=dist;
		
		// Max size of a frontier
			if (maxSizeFrontier<frontiers_[i].size)
					maxSizeFrontier=frontiers_[i].size;
			
		}
	}
	ROS_WARN("Considering %d frontiers",numSizeFrontiers);
	
	
	int countFrontier=0;
	for (int i=0; i<frontiers_.size(); i++)
	{
		if(frontiers_[i].size>min_frontier_size) // Only big frontiers
		{
			countFrontier++;

			if (std::find(bannedFrontiers.begin(), bannedFrontiers.end(), i) != bannedFrontiers.end()) // Not banned frontiers
			{
					ROS_WARN("Banned frontier %d",i);
			}
			else
			{
					ROS_WARN("NOT Banned frontier %d",i);
				...
				// Cost function
				float	coef_dist = 0.70;
				float coef_size = 0.30;
				lossFunction=coef_dist*(dist/maxDistFrontier)+coef_size*(1-frontiers_[i].size/maxSizeFrontier);
			
				ROS_WARN("Cost function for frointer %d: Dist:%f (%f/%f) Size:%f (%f/%f) Cost=%f",i,(dist/maxDistFrontier),dist,maxDistFrontier, (1-frontiers_[i].size/maxSizeFrontier), (float)frontiers_[i].size, maxSizeFrontier, lossFunction);
				// Select the frontier with the lowest cost
				if((lossFunction<minCost) && (isValidPoint(frontiers_[i].free_center_point)) )
				{
					ind_minCostFrontier=i;
					minCost=lossFunction;
				}
				
			}
			
			// If not frontier selected
			if(countFrontier==numSizeFrontiers && minCost==10000)
			{
					ROS_WARN("LAST available frointer %d with no result",i);
					bannedFrontiers.clear();
			}
		}
	...
	ROS_WARN("Selected frointer %d: Cost=%f",ind_minCostFrontier,minCost);	
	
	// Expected time to reach the position 
	float vel = 0.1;
	dif_x=frontiers_[ind_minCostFrontier].free_center_point.x-robot_pose_.position.x;
	dif_y=frontiers_[ind_minCostFrontier].free_center_point.y-robot_pose_.position.y;
	dist=pow((pow(dif_x,2)+pow(dif_y,2)),0.5);
	expectedTime = dist/vel;
	
	if (expectedTime>MAX_EXPECTED_TIME)
		expectedTime = MAX_EXPECTED_TIME;
		
	ROS_WARN("Expected Time %d",expectedTime);	
			
	// Geenerate goal pose
  geometry_msgs::Pose g;	

	g.position.x = frontiers_[ind_minCostFrontier].free_center_point.x;
	g.position.y = frontiers_[ind_minCostFrontier].free_center_point.y;
	g.position.z = frontiers_[ind_minCostFrontier].free_center_point.z;
	g.orientation = tf::createQuaternionMsgFromYaw(atan2(frontiers_[ind_minCostFrontier].center_point.y-frontiers_			[ind_minCostFrontier].free_center_point.y, frontiers_[ind_minCostFrontier].center_point.x-frontiers_[ind_minCostFrontier].free_center_point.x));

	// If not valid position, generate one valid arround 
	while (!isValidPoint(g.position))
	{
		...
		geometry_msgs::Pose goal_add=getRandomPose(0.5);
		g.position.x += goal_add.position.x;
		g.position.y += goal_add.position.y;
		g.position.z += goal_add.position.z;

		ROS_WARN("It is NOT a valid position: Generating a random pose");
		expectedTime=MAX_EXPECTED_TIME/10;
	}

	// Publsih goal
	//ROS_WARN("[%f,%f,%f,%d]",g.position.x,g.position.y,g.position.z,isValidPoint(g.position));
